...
The project implements a **Company Intelligence Agent** for **Delta Air Lines** using a retrieval-augmented generation (RAG) framework.  
...
1. **Document Collection**
   ...
   - Major shareholders summary from Yahoo Finance. 
...
2. **Data Processing Pipeline**
   ...
   - Sliding-window chunking (~600-word chunks with 100-word overlap) and storage as a unified document store (`chunks.csv`, `chunks.pkl`).  

3. **Retrieval System**
   - TF-IDF vectorization over all chunks.
   - Cosine similarity to rank and return the top-k most relevant chunks for each query. 
...
4. **LLM Integration**
   ...
   - Structured prompt that:
     ...
     - Instructs the model to say “not in the documents” when evidence is missing.

5. **KPI Extraction**
   - Semi-automated extraction of key KPIs from the 10-K and ESG report (e.g., total operating revenue, cargo revenue, YoY growth, number of segments, served customers, fuel savings, and main ESG targets), using regex search plus manual validation.  

6. **Streamlit App**
   - **Company Snapshot tab:** KPI cards, verified ESG targets, and an auto-generated 3–4 sentence overview.
   - **Ask-a-Question tab:** quick-select question groups (Snapshot, Risks, ESG, Stakeholders), user input box, LLM answer with chunk citations, and expandable retrieved-chunk table. 
...
- Identified key limitations around OCR quality, manual KPI extraction, chunking strategy, and API call limits, with recommendations for future improvements (better extraction methods, alternative chunking, and a more flexible LLM API).
